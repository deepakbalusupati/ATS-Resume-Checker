...
import {
  Box, Container, Typography, Button, TextField, Paper, CircularProgress, Switch,
  List, ListItem, ListItemText, LinearProgress, Alert, Collapse, IconButton
} from '@mui/material';
import { 
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <LinearProgress
                    ...
                    sx={{
                      height: 10,
                      mb: 2,
                      backgroundColor: result.score >= 80 ? 'success.light' :
                                      result.score >= 50 ? 'warning.light' : 'error.light'
                    }}
                  />
                  <Typography color="text.primary">
                    {result.score >= 80 ? '✅ Excellent! Your resume is ATS-friendly.' :
                     result.score >= 50 ? '⚠️ Good, but could be improved.' : 
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    <List dense>
                      {result.missingKeywords.map((keyword, i) => (
                        <ListItem key={i}>
                          <ListItemText primary={`• ${keyword}`} primaryTypographyProps={{ color: 'text.primary' }} />
                        </ListItem>
                      ))}
                    </List>
                  </Box>
                ...
                <Box>
                  ...
                  <List dense>
                    {result.suggestions.map((suggestion, i) => (
                      <ListItem key={i}>
                        <ListItemText primary={`• ${suggestion}`} primaryTypographyProps={{ color: 'text.primary' }} />
                      </ListItem>
                    ))}
                  </List>
                </Box>
